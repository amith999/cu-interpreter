...
class Interpreter(object):
	...
	def handle_print_statements(self):
		...
		#print "FINAL RES " + str(expression_val)
	...
	def evaluate(self):
		...
		if self.lex_list[self.pos] == "print":
		...
		elif self.lex_list[self.pos].isalpha():
			...
			temp_pos = self.pos
			print "BOSS " + str(variable_map)
			self.pos = self.pos + 2
			self.handle_assignment_statements (temp_pos)
			print "BOSS 1 " + str(variable_map)
			## to confirm if this is an assignment operation
		...
		while self.pos < num_ops:
			...
			if self.lex_list[self.pos].isalpha():
				if self.lex_list[self.pos] not in variable_map:
					...
					print str(var)
					print "VARRRRR " + str(variable_map[var])
					self.lex_list[self.pos] = variable_map[var]
